...
//--------------------------------TCP推送通道 查询接口类---------------------------------------------
/********************快照查询Spi（现货、指数、港股通、期权、期货、港股委托挂单、港股经纪商席位、互联网和托管机房模式通用spi）********************/
...
public:
    ...
    /**
     * @brief OnMDHKExOrderSnapshot    商业港股委托挂单数据回调
     *
     * @param hke_order_snapshot       商业港股委托挂单数据
     * @param cnt                      数据个数
     * @attention                      使用后需要通过接口 IGMDApi::FreeMemory释放数据
     */    
    virtual void OnMDHKExOrderSnapshot(MDHKExOrderSnapshot* hke_order_snapshot, uint32_t cnt)
    { IGMDApi::FreeMemory(hke_order_snapshot); (void)cnt; }

    /**
     * @brief OnMDHKExOrderBrokerSnapshot    经纪商席位数据回调
     *
     * @param hke_order_broker_snapshot      经纪商席位数据
     * @param cnt                            数据个数
     * @attention                            使用后需要通过接口 IGMDApi::FreeMemory释放数据
     */    
    virtual void OnMDHKExOrderBrokerSnapshot(MDHKExOrderBrokerSnapshot* hke_order_broker_snapshot, uint32_t cnt)
    { IGMDApi::FreeMemory(hke_order_broker_snapshot); (void)cnt; }
...
class IGMDETFInfoSpi
{
public:
    virtual ~IGMDETFInfoSpi() {}

    /**
     * @brief OnMDETFInfo           接收ETF代码表查询数据回调
     *
     * @param etf_info              ETF代码表信息数据
     * @param cnt                   数据个数
     * @attention                   使用后需要通过接口 IGMDApi::FreeMemory释放数据
     */ 
    virtual void OnMDETFInfo(MDETFCodeTableRecord* etf_info, uint32_t cnt)
    { IGMDApi::FreeMemory(etf_info); }

    /**
     * @brief OnStatus              异步查询状态数据回调
     *
     * @param status                查询状态数据
     * @attention                   使用后需要通过接口 IGMDApi::FreeMemory释放数据
     */  
